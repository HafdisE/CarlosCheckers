...
int LSB(int32 x)
	...
	if(_BitScanForward((DWORD*)&returnvalue,x))
		return returnvalue;
	else
		return -1;
...
int returnvalue;
	...
	if(_BitScanReverse((DWORD*)&returnvalue,x))
		return returnvalue;
	else
		return -1;
...
int db_init(int suggestedMB, char out[256])
	...
	*/	
	...
	cachebaseaddress = (unsigned char*)VirtualAlloc(0,memsize,MEM_RESERVE|MEM_COMMIT|MEM_TOP_DOWN,PAGE_READWRITE);
	...
	blockpointer = (unsigned char**)VirtualAlloc(0,memsize,MEM_RESERVE|MEM_COMMIT|MEM_TOP_DOWN, PAGE_READWRITE);
	...
	blockinfo = (bi*)VirtualAlloc(0,memsize, MEM_RESERVE|MEM_COMMIT|MEM_TOP_DOWN, PAGE_READWRITE);
